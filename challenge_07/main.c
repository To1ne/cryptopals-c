...
void decrypt_aes_ecb(unsigned char* data, size_t data_len, unsigned char* key, size_t key_len)
{
   int ret = 0;
   aes_context ctx;
   unsigned char decr[16];
   int i = 0;
   ...
   for (i = 0; i < data_len; i += 16) {
      int j = 0;
      ret = aes_crypt_ecb(&ctx, AES_DECRYPT, &data[i], decr);
      if (ret) {
...
int main(int argc, char *argv[])
{
   ...
   // read file to buffer
   data = read_file("7.txt", &data_len);

   // convert base64 to binary data
   base64_to_bin(data, &data_len);

   // decrypt
   decrypt_aes_ecb(data, data_len, key, sizeof(key)-1 );
